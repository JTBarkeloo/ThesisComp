...
\begin{itemize}
\item Using keras on top of tensorflow various input parameters are tested for model behavior
\item Networks are set up with 1 input layer, 2 hidden layers with 10 nodes (+1 bias node), and 1 output node
...
\begin{itemize}
\item Follow up changes allow a better limit with a cut that is less tempermental and harsh (0.92/0.95 instead of 0.98)
...
\begin{itemize}
\item New tools have been recently developed in the Top Group (VORGamma, Duplicate Event Removal)
\item Replacing Custom Event Saver with that of tt+gamma group, more support
