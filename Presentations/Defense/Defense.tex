...
\item Very Minor Backgrounds: Diboson, tt+V
\end{itemize}
\centering
...
\begin{column}{0.6\textwidth}
\begin{itemize}
\item Most systematics do not effect fit
...
\item Each of these between 1.5-2.5\% effect relative to nominal uncertainty
\end{itemize}
...
\begin{itemize}
\item Limits have been set on the process $t\rightarrow q \gamma$, the best current limit on $t\rightarrow c\gamma$
\item Neural network implementation improved result by up to 30\% compared to stats only improvement from 8 TeV result
