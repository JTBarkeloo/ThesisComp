...
\begin{figure}[h!]
\centering
\subfloat[electron channel]{\includegraphics[width=.45\columnwidth]{../ThesisImages/RegionPlots/BeforeScaling/PreSelection/FCNC_All_ejets/Plots/PreSel_ST.png}}\hfil
\subfloat[muon channel]{\includegraphics[width=.45\columnwidth]{../ThesisImages/RegionPlots/BeforeScaling/PreSelection/FCNC_All_mujets/Plots/PreSel_ST.png}}
\caption{$S_T$ in the signal region pre-selection region.  FCNC signal branching ratio is scaled to 1\%}
\label{fig:PreSelPlots1}
\end{figure}
...
In order to remove photon contributions from initial or final state radiation processes events are removed where the photon candidate is too close to another object
%%% Photon DeltaR plots
% DelR gam l
% DelR gam j
...
\subfloat[]{\includegraphics[width=.33\columnwidth]{../ThesisImages/RegionPlots/BeforeScaling/PreSelection/FCNC_All_ejets/Plots/PreSel_ph_pt.png}} \hfil  %Replace with JET Change
...
\vspace{-3.mm}
\subfloat[]{\includegraphics[width=.33\columnwidth]{../ThesisImages/RegionPlots/BeforeScaling/PreSelection/FCNC_All_ejets/Plots/PreSel_MWT.png}}\hfil
...
\caption{Photon $p_T$ (a), leading light jet $p_T$ (b), lepton $p_T$ (c), $m_T^W$(d), $\slashed{E}_T$ (e), and $n_{\text{jets}}$ (f) plots in the signal region pre-selection for the electron+jets channel.  FCNC signal branching ratio is scaled to 1\% }
...
\subfloat[]{\includegraphics[width=.33\columnwidth]{../ThesisImages/RegionPlots/BeforeScaling/PreSelection/FCNC_All_mujets/Plots/PreSel_ph_pt.png}}\hfil  % Replace with JET Change
...
\vspace{-3.mm}
\subfloat[]{\includegraphics[width=.33\columnwidth]{../ThesisImages/RegionPlots/BeforeScaling/PreSelection/FCNC_All_mujets/Plots/PreSel_MWT.png}}\hfil
...
\caption{Photon $p_T$ (a), leading light jet $p_T$ (b), lepton $p_T$ (c), $m_T^W$(d), $\slashed{E}_T$ (e), and $n_{\text{jets}}$ (f) plots in the signal region pre-selection for the muon+jets channel.  FCNC signal branching ratio is scaled to 1\%}
...
\caption{Distance between the photon and closest light jet: $\Delta R_{\gamma j}$ in the signal region pre-selection region.  FCNC signal branching ratio is scaled to 1\%}
...
\label{sec:BKGnoPho}
Various background processes that do not have a real photon produced in the events can still enter the signal region if an electron or jet is mis-reconstructed as a photon.  Of these processes the largest contributors in the signal region are Standard Model $t\bar{t}$ and W+jets.  Designing a single control region satisfactorily close to the signal region is impossible.  Thus, two control regions are designed, one which is W+jets rich and the other $t\bar{t}$ rich.  Scale factors for these backgrounds are derived simultaneously and tested in a third similar region for validation before being applied to other regions.  
...
\caption{$S_T$ distributions in the 3(a,d), 4(b,e) ,5+(c,f) jets control and validation regions. Electron channel is shown on the top and the muon channel on the bottom}
\label{fig:CRSTs}
\end{figure}
