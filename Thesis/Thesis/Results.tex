...
The data and MC yields after fitting are presented in Table \ref{tab:postfitSRYields}.  The signal sample, due to its shape with respect to the data, is fitted away.  This implies that there is no excess in data events that are similar to the signal MC.  This fitting methodology is continued in Section \ref{sec:Limits} to set experimental upper bounds on the process $t\rightarrow q \gamma$ taking into consideration all theoretical and experimental systematic uncertainties.
\begin{table}[!htb]
\begin{center}
